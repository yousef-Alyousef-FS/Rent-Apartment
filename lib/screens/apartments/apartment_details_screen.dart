...
import 'package:sakani/screens/booking/booking_screen.dart';
...
class ApartmentDetailsScreen extends StatefulWidget {
  ...
  const ApartmentDetailsScreen(
      {super.key, required this.apartment, this.isOwnerView = false});
...
class _ApartmentDetailsScreenState extends State<ApartmentDetailsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        ...
        body: ListView(children: [
          _buildImageCarousel(context, apartment.images),
          Padding(
                    ...
                    Row(
                        ...
                        children: [
                          _buildDetailIcon(context, Icons.bed_outlined,
                              loc.rooms(apartment.rooms ?? 0)),
                          _buildDetailIcon(
                              context,
                              Icons.area_chart_outlined,
                              '${apartment.area ?? 0} ${loc.sqm}')
                        ]),
                    const Divider(height: 32),
        ...
        bottomNavigationBar: widget.isOwnerView
            ? _buildOwnerActionsBar(theme, loc)
            : _buildBookingBar(theme, loc));
  }
          ...
          Column(
              ...
              children: [
                ...
                Text.rich(TextSpan(children: [
                  TextSpan(
                      text: '\$${widget.apartment.price.toStringAsFixed(0)}',
                      style: theme.textTheme.titleLarge
                          ?.copyWith(fontWeight: FontWeight.bold)),
                  TextSpan(
                      text: loc.perNight, style: theme.textTheme.bodyMedium)
                ]))
              ]),
          ElevatedButton(
  ...
  Widget _buildImageCarousel(
      BuildContext context, List<ApartmentImage> images) {
    if (images.isEmpty) {
      return Container(
          height: 250,
  ...
  Widget _buildRatingSection(
      ThemeData theme, AppLocalizations loc, Apartment apartment) {
    ...
    return Row(
        children: [...stars, const SizedBox(width: 8), Text('$rating (${loc.reviews(reviewCount)})')]);
  }
